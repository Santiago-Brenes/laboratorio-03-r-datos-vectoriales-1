...
```{r}
kable(world[1:10, 1:5, drop = TRUE], 
...
```{r}
kable(world[1:10, c("name_long","continent","pop","lifeExp","gdpPercap"), drop = TRUE], 
...
kable(world.ordenadoxpais[, c("name_long","continent","pop","lifeExp","gdpPercap"), drop = TRUE], 
  col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),  
  caption="Datos demográficos de los países del mundo") %>%
kable_styling() %>%
scroll_box(
...
# Se formatea la tabla con el nuevo data frame
kable(centro_america[, c("name_long","continent","pop","lifeExp","gdpPercap"), drop = TRUE], 
  col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),  
...
```{r}
ggplot(centro_america, aes(x=reorder(name_long,-lifeExp), y=lifeExp)) + ggtitle("Esperanza de vida de los países de Centroamérica") + xlab("País") + ylab("Esperanza de vida en años") +  geom_col() + geom_text(aes(label=lifeExp), vjust=-0.3, size=3.5)
```
...
```{r}
plot(centro_america["lifeExp"],main = "Esperanza de vida de los países de Centroamérica")
```
...
```{r}
centro_america_esperanzadevida = centro_america[centro_america$lifeExp >= 74,]

plot(centro_america_esperanzadevida["lifeExp"],main = "Esperanza de vida de 74 años o más en países de Centroamérica")
```